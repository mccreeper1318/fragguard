...
The schema-v3 to schema-v4 migration is different: the persistent same-tick coalescing index is derived data and is no longer needed. FragGuard verifies the live database, transactionally removes `idx_fg_tick_coalesce` if it still exists, verifies the database again, and only then advances `PRAGMA user_version` to 4. It does **not** create a full history backup and does **not** build a replacement multi-gigabyte index. A retry from schema v3 where a previous attempt already removed the index is explicitly supported.
